...
export interface ExpandCollapseAction {
  section: Exclude<ProjectSection, 'header' | 'description'>;
...
interface Props {
  ...
  onCoIConfirm: () => void;
  coiLoading: boolean;
  dispatchAction: (section: ExpandCollapseAction['section'], action: boolean) => void;
  action: ExpandCollapseAction | undefined;
  dispatchAutoScrollAction: (section: ProjectSection) => void;
  autoScrollAction: AutoScrollAction | undefined;
  name: string;
  sectionExpanded: Record<Exclude<ProjectSection, 'header' | 'description'>, boolean>;
...
export const ProjectCard: React.FC<Props> = ({
  ...
  const handleAutoScroll = useCallback((visibleIds: string[]) => {
    // console.log('visible ids in', name, visibleIds);
    const sections = Object.values(ProjectSection);
    const effectiveIds = visibleIds.filter((el) => sections.some((item) => el.includes(item)));
    // console.log('effective ids in', name, effectiveIds);
    const lastSection = effectiveIds.pop()?.split('-')[0] as ProjectSection;

    if (lastSection) dispatchAutoScrollAction(lastSection);
  }, [dispatchAutoScrollAction]);
  ...
  const handleSectionClick = (id: ExpandCollapseAction['section'], expanded: boolean) => () => {
